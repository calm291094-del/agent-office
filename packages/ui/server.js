// packages/ui/server.js - Versión original funcional
...
app.get('/', (req, res) => {
    ...
    html += 'async function crearAgente() { const name = prompt("Nombre del agente:", "Agente " + (agents.length + 1)); if (!name) return; const role = prompt("Rol del agente:", "Asistente"); try { await fetch("/api/agents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, role: role || "Asistente" }) }); await cargarAgentes(); agregarChat("Sistema", "✅ Agente \\"" + name + "\\" creado"); } catch(e) { alert("Error al crear agente"); } }';
    html += 'async function eliminarAgente(id) { if (!confirm("¿Eliminar este agente?")) return; try { await fetch("/api/agents/" + id, { method: "DELETE" }); await cargarAgentes(); agregarChat("Sistema", "🗑️ Agente eliminado"); } catch(e) { alert("Error al eliminar"); } }';
